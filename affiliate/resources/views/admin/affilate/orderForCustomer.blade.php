...
@section('mainContent')
    ...
    <div class="box-body" style="background: #f2f2f2;">
        ...
        <div class="row" id="link_products">
            @include('admin.affilate.orderForCustomer_pagination')
        </div>
        <style>
            .name{
                height: 33px;
    ...
    <script>
        $(document).ready(function(){
            ...
            function fetch_data(page, query)
            {
                $.ajax({
                    type:"GET",
                    url:"{{url('orderforCustomer/products/pagination')}}?page="+page+"&query="+query,
            ...
            $(document).on('change', '#category_id', function(){
                ...
                if(query.length >0) {
                    $.ajax({
                        type:"GET",
                        url:"{{url('orderForCustomer/pagination/category')}}?page="+page+"&query="+query,
                        success:function(data)
                        {
                            $('#link_products').html('');
                            $('#link_products').html(data);
